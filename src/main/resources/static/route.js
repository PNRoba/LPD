...
function renderRoutes(solution, indictments) {
    ...
    const bus_div = $("#bus_container");
    solution.busList.forEach((bus) => {
        console.log(indictmentMap);
        var v_badge = "badge bg-danger";
        if (indictmentMap[bus.name]==null || getHardScore(indictmentMap[bus.name].score)==0) { v_badge = "badge bg-success"; }
        bus_div.append($('<a data-toggle="popover" data-bs-html="true" data-bs-content="' +
        'capacity=' + bus.capacity +
        '<hr>' +
        getEntityPopoverContent(bus.name, indictmentMap) +
        '" data-bs-original-title="'+ bus.name + ' (' + bus.capacity + ')' +'"><span class="'+ v_badge +'">'+
        bus.name + ' (' + bus.capacity + ')' +'</span></a>'));
        var visit_nr = 1;
        bus.visits.forEach((visit) => {
